        ...
        {rows.length === 0 ? (
          ...
          <View>
            <View style={styles.tableHeader}>
              <View style={{ width: '16%' }}><Text style={styles.th}>Receipt No.</Text></View>
              <View style={{ width: '20%' }}><Text style={styles.th}>Donor</Text></View>
              <View style={{ width: '18%' }}><Text style={styles.th}>Type</Text></View>
              <View style={{ width: '14%' }}><Text style={styles.th}>Amount</Text></View>
              <View style={{ width: '14%' }}><Text style={styles.th}>Mode</Text></View>
              <View style={{ width: '18%' }}><Text style={styles.th}>Date</Text></View>
            </View>
            {rows.map((r, i) => (
              <View key={r.id || i} style={styles.tableRow}>
                <View style={{ width: '16%' }}><Text style={styles.td}>{r.receipt_no}</Text></View>
                <View style={{ width: '20%' }}><Text style={styles.td}>{r.donor_name}</Text></View>
                <View style={{ width: '18%' }}><Text style={styles.td}>{r.donation_type}</Text></View>
                <View style={{ width: '14%' }}><Text style={styles.td}>₹{Number(r.amount).toLocaleString('en-IN')}</Text></View>
                <View style={{ width: '14%' }}><Text style={styles.td}>{r.payment_mode}</Text></View>
                <View style={{ width: '18%' }}><Text style={styles.td}>{fmtDate(r.created_at)}</Text></View>
              </View>
            ))}
          </View>
        )}
